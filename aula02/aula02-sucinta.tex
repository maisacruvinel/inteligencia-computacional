...
%%
%% Compilar:  latexmk -xelatex aula02-sucinta.tex   (duas passadas!)
%% ============================================================
...
\begin{frame}{O problema desta aula: representação}
  ...
  \smallskip
  Antes de treinar (próxima aula): existe conjunto de pesos que computa a
  função desejada?
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      \begin{ideiabox}
        Camada oculta representa tudo — mas pode exigir largura
...
\begin{frame}{O neurônio de limiar é uma porta lógica}
  ...
  Entradas em $\{0,1\}$ e degrau: $\vw$ e o limiar $T$ dão as portas
  clássicas — e a de maioria:
  ...
  \smallskip
  \begin{ideiabox}
    Maioria: \textbf{um} neurônio; em circuitos AND/OR/NOT de profundidade
    fixa, exponencial. Neurônios de limiar = portas mais fortes que as
    booleanas.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{XOR exige camada oculta}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \begin{alertabox}
        Da aula 0: um único perceptron não computa o XOR — classes não
        linearmente separáveis.
      \end{alertabox}

      \smallskip
      \begin{itemize}\setlength{\itemsep}{0.2ex}
        \item Camada oculta re-representa a entrada (computa OR e AND)
...
\begin{frame}{O pior caso: paridade}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.6\textwidth}
      ...
      camada $K$ exige $\mathcal{O}(2^{cN})$ neurônios.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Limites do argumento}
  ...
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    ...
    \item Hipótese do \emph{deep learning}: visão, fala e linguagem têm
          essa estrutura
    \item Custo real = número de \textbf{conexões} (pesos), não de
...
\begin{frame}{No contínuo, o perceptron é um hiperplano}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \smallskip
      \begin{itemize}\setlength{\itemsep}{0.2ex}
        \item Portas booleanas = este desenho com entradas em $\{0,1\}^2$
...
\begin{frame}{Profundidade, de novo: o tabuleiro de 16 retas}
  ...
  \medskip
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    \item Em 2D, custo quadrático: só 2 dimensões independentes
    \item Dimensão $D$: pior caso raso
...
\begin{frame}{Demonstração: enunciado e estratégia}
  ...
  \smallskip
  Três passos, orçamento de erro $\epsilon/2 + \epsilon/2$:
...
\begin{frame}{Passo 1: continuidade uniforme dá a partição}
  ...
  \smallskip
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item O $\delta$ vale para o intervalo \textbf{inteiro} — o que o
          compacto compra; em $\R$ todo, $f(x)=x^2$ não teria $\delta$ único
    \item Particione: $a = T_0 < T_1 < \cdots < T_K = b$ com
          $T_{i+1} - T_i < \delta$; $K = \lceil (b-a)/\delta \rceil$ janelas
...
\begin{frame}{Passo 2: a rede de degraus erra menos que $\epsilon/2$}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Pulsos não se sobrepõem: em $[T_i, T_{i+1})$ vale
...
\begin{frame}{Passo 3: trocar o degrau pela sigmoide}
  ...
  $c_i$; resultado: $g$. E $\sig(k z) \to u(z)$ quando $k \to \infty$,
  para todo $z \ne 0$.

  \smallskip
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item \textbf{Longe dos limiares} ($\abs{x - T_i} \ge \Delta$ para todo
          $i$): cada termo erra no máximo $\sig(-k\Delta)$; erro total
          $\le K \max_i \abs{c_i}\, \sig(-k\Delta) \to 0$ em $k$. Escolha
          $k$ que o torne $< \epsilon/2$
    \item \textbf{Perto de um limiar} $T_i$: $g$ fica, a menos desse mesmo
          erro, entre os patamares vizinhos $f(T_{i-1})$ e $f(T_i)$ — ambos
          distam menos de $\epsilon/2$ de $f(x)$, pois a janela é curta
          (Passo 1)
  \end{itemize}
...
\begin{frame}{Suficiência de arquitetura}
  ...
  Rede representa a função-alvo só com capacidade: largura \textbf{e}
  profundidade suficientes. No tabuleiro de 16 retas:
  ...
  \begin{itemize}\setlength{\itemsep}{0.25ex}
    ...
    \item Com degrau, a 1ª camada \alert{apaga informação}: regiões
          distintas colapsam na mesma saída binária — posteriores não
          recuperam
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35ex}
        ...
        \item Versão rasa pode custar largura exponencial (paridade,
              dimensão da entrada)
