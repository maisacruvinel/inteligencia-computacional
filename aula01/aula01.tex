...
%% círculos IA/AM/DL, por que agora, forma matricial, ativações,
%% perda e paradigmas de aprendizado.
%%
%% Compilar:  latexmk -xelatex aula01.tex   (duas passadas!)
%%
%% Imagens e links complementares: ver plano-aula1-imagens-e-links.md;
%% baixe/atualize as fotos com  imagens/baixar-imagens.sh
%% ============================================================
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.56\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.35ex}
        \item {\color{icNeural}\textbf{Inteligência artificial}} — qualquer
              técnica que faça o computador imitar comportamento humano
        \item {\color{icAzulClaro}\textbf{Aprendizado de máquina}} — a
              habilidade de aprender sem ser explicitamente programado
        \item {\color{icMagenta}\textbf{Aprendizado profundo}} — extrair
              padrões dos dados brutos com redes de múltiplas camadas
      \end{itemize}
    \end{column}
  \end{columns}

  \begin{ideiabox}
    A frase-resumo: ensinar o computador a aprender a tarefa a partir de dados
...
\begin{frame}{Por que só agora?}
  ...
  \begin{alertabox}
    As ideias centrais desta aula são dos anos 1950--1980. O que mudou foi a
    escala — e escala, aqui, muda o que é possível, não apenas o quão rápido.
  \end{alertabox}
\end{frame}
...
\begin{frame}{O que uma rede profunda fez pela biologia}
  ...
  \vspace{-0.75em}
  \begin{ideiabox}
    Por dentro é a receita desta disciplina: modelo diferenciável de ponta
    a ponta, treinado por gradiente — e feito de \emph{atenção}, onde o
    curso termina.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Os rostos desta história}
  ...
  \vspace{-0.35em}
  \begin{ideiabox}
    Dois marcos esquecidos na linha do tempo: o ADALINE (1960) e a regra
    delta de Widrow--Hoff — a avó da descida de gradiente da aula 2.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{O desenho que começou tudo}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.62\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.25ex}
        \item \textbf{1906:} Cajal recebe o Nobel de Medicina pela
              \alert{doutrina do neurônio} — células individuais que se
              comunicam por \alert{sinapses}
        \item \textbf{1943:} McCulloch \& Pitts reduzem o desenho a somar
              entradas ponderadas e comparar com um limiar
        \item De um desenho de beleza enciclopédica sobra um modelo de
              poucas linhas — e é esse modelo que ainda sustenta o deep
              learning
      \end{itemize}
      \vspace{-0.7em}
      \begin{ideiabox}
        A história da área é uma sucessão de \emph{simplificações felizes} —
        cada uma descarta detalhe biológico e ganha poder de cálculo.
      \end{ideiabox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Do vetor ao lote: a forma matricial}
  ...
  \begin{itemize}\setlength{\itemsep}{0.35ex}
    \item O viés $w_0$ desloca a fronteira de decisão — sem ele, ela é
          obrigada a passar pela origem
    \item Com um lote de $B$ exemplos, a entrada vira uma matriz: $X$ é
          $B{\times}m$, $W$ é $m{\times}1$ e a saída, $B{\times}1$ — o lote
          atravessa a conta intocado
    \item Tudo o que um perceptron faz é um produto interno seguido de uma
          função escalar — exatamente a operação que as GPUs fazem bem
  \end{itemize}

  \begin{ideiabox}
    Uma rede profunda é, em essência, uma pilha de multiplicações de matrizes
    intercaladas com não linearidades. Erros de dimensão são a fonte número
    um de bugs: as colunas da entrada casam com as linhas dos pesos — sempre.
  \end{ideiabox}
\end{frame}
...
\begin{frame}{Propagação direta, passo a passo}
  ...
  \begin{alertabox}
    Repare no passo 2: a ReLU zerou a segunda unidade oculta. Para esta
    entrada ela não contribui em nada — e, na retropropagação, não receberá
    gradiente. É assim que a ReLU produz \alert{ativações esparsas}.
  \end{alertabox}
\end{frame}
...
\begin{frame}{Os paradigmas de aprendizado}
  ...
  \begin{ideiabox}
    Nas quatro linhas da tabela, a rede é a mesma e o gradiente é calculado
    por retropropagação. Um modelo de linguagem moderno usa três delas em
    sequência: pré-treino auto-supervisionado, ajuste supervisionado e
    alinhamento por reforço (tema da disciplina GBC063).
  \end{ideiabox}
\end{frame}
