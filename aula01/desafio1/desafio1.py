...
Alunos:
Maisa Campos Cruvinel - 12411BCC071
Evandro Flausino - 12311BCC074
...
class Submissao:
    """
    Random Fourier Features (RFF) para o kernel RBF (Rahimi & Recht, 2007).

    Ideia: um kernel gaussiano k(x, x') = exp(-||x-x'||^2 / (2*sigma^2)) pode
    ser aproximado por um produto interno em um espaço de features aleatório:

        phi(x) = sqrt(2/D) * cos(W^T x + b),   W ~ N(0, 1/sigma^2), b ~ U[0, 2*pi)

    de modo que phi(x)^T phi(x') ≈ k(x, x'). Isso permite que o Perceptron
    (que só separa por hiperplanos) aprenda fronteiras não lineares, pois no
    espaço de features os dados ficam aproximadamente linearizados pelo kernel.

    Escolha de sigma: usamos a heurística da mediana das distâncias par a par
    (Gretton et al.) sobre uma amostra dos dados de treino padronizados — ela
    adapta a escala automaticamente para qualquer dataset, sem "espiar" rótulos
    nem ajustar manualmente para uma tarefa específica.
    """
    DIM_MAX = DIM_MAX

    def fit(self, X: torch.Tensor) -> None:
        """Vê só as entradas de treino (sem rótulos): padroniza e sorteia as
        projeções aleatórias W, b da aproximação de Fourier do kernel RBF."""
        g = torch.Generator().manual_seed(0)          # sorteio determinístico

        # padronização (cada dimensão fica com média 0 e desvio 1)
        self.mu, self.sd = X.mean(0), X.std(0) + 1e-8
        Xs = (X - self.mu) / self.sd

        # heurística da mediana: escolhe sigma na escala típica dos dados
        amostra = Xs[:300]
        d2 = torch.cdist(amostra, amostra) ** 2
        pos = d2[d2 > 0]
        sigma = math.sqrt(pos.median().item() / 2) if len(pos) > 0 else 1.0
        sigma = max(sigma, 1e-3)                       # evita divisão por ~0

        d = X.shape[1]
        self.W = torch.randn(d, DIM_MAX, generator=g) / sigma   # (d, 64)
        self.b = torch.rand(DIM_MAX, generator=g) * 2 * math.pi  # (64,)

    def phi(self, X: torch.Tensor) -> torch.Tensor:
        """(n, d) -> (n, 64). Determinística dado self.W, self.b (fixados no fit)."""
        Xs = (X - self.mu) / self.sd
        return math.sqrt(2 / DIM_MAX) * torch.cos(Xs @ self.W + self.b)
