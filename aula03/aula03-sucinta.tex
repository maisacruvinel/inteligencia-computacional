...
%%
%% Compilar:  latexmk -xelatex aula03-sucinta.tex   (XeLaTeX, duas passadas)
%% ============================================================
...
\begin{frame}{Roteiro da aula}
  ...
  \begin{itemize}\setlength{\itemsep}{0.5ex}
    \item<1-> \textbf{O problema de aprendizado:} rede $=$ função paramétrica
          $f(\vx; W)$; aprender $=$ estimar $W$ de \emph{amostras}
    \item<2-> \textbf{Histórico:} regra do perceptron — e por que ela
    ...
    \item<4-> \textbf{ERM:} o arcabouço de todo treinamento em PyTorch
...
\begin{frame}{Recapitulação: redes representam qualquer função}
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item Redes neurais: \textbf{aproximadores universais} — funções booleanas,
    ...
    \item Voz $\to$ transcrição, imagem $\to$ legenda, estado do jogo $\to$ jogada:
          todas funções — logo, todas modeláveis por uma rede
  \end{itemize}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{itemize}\setlength{\itemsep}{0.3ex}
        \item A arquitetura (camadas, conexões): \emph{decisão de projeto} —
              assumimos que ela basta
        \item Os \textbf{parâmetros} $W$: o que o treinamento ajusta
        \item Rede direta (\emph{feedforward}): sem laços — realimentação
              fica para o futuro
      \end{itemize}
    \end{column}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      ...
      \begin{ideiabox}
        ...
        para isso seria preciso conhecer $g$ em todo lugar; e aí não haveria
        o que aprender.
      \end{ideiabox}
    \end{column}
...
\begin{frame}{Amostrar a função: os dados de treinamento}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Ajustamos $W$ para que $f(\vx_i; W) \approx d_i$ \emph{nos pontos
          amostrados}
...
\begin{frame}{Quando funciona — e quando trava}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    ...
    \item Intuição do limitante: cada erro dá um passo de tamanho $\sim\!R$,
          e são precisos muitos passos de tamanho $\gamma$ para desfazê-lo
  \end{itemize}
...
\begin{frame}{Curiosidade histórica: ADALINE, MADALINE e a regra delta}
  ...
  \begin{ideiabox}
    ...
    ativação diferenciável e encadeada camada a camada, ela vira a
    ...
    \begin{column}{0.55\textwidth}
      \begin{itemize}\setlength{\itemsep}{0.4ex}
        ...
        \item Mexer um pouquinho em um peso \alert{não altera o erro} — não há
              sinal de \emph{qual direção} melhora; o erro vira função em
              platôs: sem gradiente, sem bússola
      \end{itemize}
    \end{column}
    \begin{column}{0.41\textwidth}
      \begin{center}
...
\begin{frame}{Divergência: medindo o desajuste de forma diferenciável}
  ...
  \begin{itemize}\setlength{\itemsep}{0.3ex}
    \item Classificação: \alert{não} é o erro $0/1$ — é um substituto suave
          dele (veremos: entropia cruzada)
    \item Regressão: o clássico $\dive = \norm{f(\vx;W) - g(\vx)}^2$
    \item A escolha da divergência: decisão de modelagem tão importante
...
\begin{frame}{Risco esperado, risco empírico}
  ...
  (perda); fixado o conjunto de treinamento, a perda é função apenas de $W$.}
\end{frame}
...
\begin{frame}{Treinar $=$ otimizar: a ponte para a próxima aula}
  \begin{itemize}\setlength{\itemsep}{0.4ex}
    \item Enunciado puramente matemático: dado $\D$, minimizar
          $\hat{R}(W)$ em relação a $W$ — \textbf{minimização de funções}
    \item Tudo diferenciável $\Rightarrow$ o gradiente
